...
# wandb.init(project="usi3d", entity="garnet0106")
...
def get_intrinsic_data_loader(conf, is_sup=False, rate=None):
    ...
    train_dict = {'input': 'train-input.txt',
                    'edge-input': 'train-edge-dexi.txt',
                    'albedo': 'train-reflectance{}.txt'.format(rate),
                    'edge-albedo': 'train-edge_reflectance_dexi.txt',
    ...
    train_loader = DataLoader(train_set, num_workers=conf['num_workers'], shuffle=True, batch_size=1)
...
def write_loss(iterations, trainer, train_writer):
    ...
    # wandb.log({key: getattr(trainer, key) for key in members})
    for m in members:
