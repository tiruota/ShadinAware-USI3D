...
class UnsupIntrinsicTrainer(nn.Module):
    def __init__(self, param):
        ...
        self.gen_r = AdaINGen(param['input_dim_b'], param['input_dim_b'], param['gen'], 'reflectance')  # auto-encoder for domain R
        ...
        self.use_ssim_loss = True # additional
        self.use_sobel_loss = True # additional
    ...
    def inference(self, x_i, x_e, use_rand_fea=False):
        with torch.no_grad():
            c_i, s_i_fake = self.gen_i.encode(x_i)
            if self.with_mapping:
                s_r, s_s = self.fea_s(s_i_fake)
            ...
            if use_rand_fea:
                s_r = Variable(torch.randn(x_i.size(0), self.style_dim, 1, 1).cuda()) + self.bias_shift
                s_s = Variable(torch.randn(x_i.size(0), self.style_dim, 1, 1).cuda()) - self.bias_shift
            x_ri = self.gen_r.decode(c_i, s_r, x_e)
            x_si = self.gen_s.decode(c_i, s_s)
        return x_ri, x_si

    # noinspection PyAttributeOutsideInit
    def gen_update(self, x_i, x_e_i, x_r, x_e_r, x_s, targets=None, param=None):
        ...
        x_s_recon = self.gen_r.decode(c_s, s_s_prime, x_e_r)
        # decode (cross domain)
        x_rs = self.gen_r.decode(c_s, s_r_rand, x_e_r)
        x_ri = self.gen_r.decode(c_i, s_ri, x_e_i)
        x_ri_rand = self.gen_r.decode(c_i, s_r_rand, x_e_i)
        ...
        x_rsr = self.gen_r.decode(c_sr_recon, s_r_prime, x_e_r)
        ...
        self.loss_kl_r = self.kl_div_loss(nn.functional.log_softmax(s_ri, dim=1),
                                          nn.functional.softmax(s_r_prime, dim=1)) if self.with_mapping and ('kl_w' in param) else 0
        self.loss_kl_s = self.kl_div_loss(nn.functional.log_softmax(s_si, dim=1),
                                          nn.functional.softmax(s_s_prime, dim=1)) if self.with_mapping and ('kl_w' in param) else 0
        ...
        # Reflectance SSIM loss
        self.loss_refl_ssim_ri = pytorch_ssim.ssim(self.normalize_min_max(x_ri), x_i).data if self.use_ssim_loss is True else 0
        self.loss_refl_ssim_ri_rand = pytorch_ssim.ssim(self.normalize_min_max(x_ri_rand), x_i).data if self.use_ssim_loss is True else 0

        # Reflectance sobel edge loss
        self.loss_refl_sobel_ri = self.bce_loss(self.compute_sobel_edge(self.normalize_min_max(x_ri)), x_e_i) \
                                  if self.use_sobel_loss is True else 0
        self.loss_refl_sobel_ri_rand = self.bce_loss(self.compute_sobel_edge(self.normalize_min_max(x_ri_rand)), x_e_i) \
                                  if self.use_sobel_loss is True else 0
        # total loss
        self.loss_gen_total = param['gan_w'] * self.loss_gen_adv_rs + \
                              ...
                              param['ssim_w'] * self.loss_refl_ssim_ri + \
                              param['ssim_w'] * self.loss_refl_ssim_ri_rand + \
                              param['sobel_w'] * self.loss_refl_sobel_ri + \
                              param['sobel_w'] * self.loss_refl_sobel_ri_rand

        self.loss_gen_total.backward()
        self.gen_opt.step()

    def sample(self, x_i, x_e_i, x_r, x_e_r, x_s):
        self.eval()
        ...
        for i in range(x_i.size(0)):
            ...
            x_r_recon.append(self.gen_r.decode(c_r, s_r_fake, x_e_r[i].unsqueeze(0)))
            x_s_recon.append(self.gen_s.decode(c_s, s_s_fake))
            x_rs.append(self.gen_r.decode(c_s, s_r[i].unsqueeze(0), x_e_r[i].unsqueeze(0)))
            x_ri.append(self.gen_r.decode(c_i, s_ri.unsqueeze(0), x_e_i[i].unsqueeze(0)))
    ...
    # noinspection PyAttributeOutsideInit
    def dis_update(self, x_i, x_e_i, x_r, x_e_r, x_s, params):
        ...
        if self.with_mapping:
            ...
            s_si = Variable(torch.randn(x_i.size(0), self.style_dim, 1, 1).cuda()) - self.bias_shift
        # decode (cross domain)
        x_rs = self.gen_r.decode(c_s, s_r, x_e_r)
        x_ri = self.gen_r.decode(c_i, s_ri, x_e_i)
    ...
    def normalize_min_max(self, x, min_val=0.0, max_val=1.0, eps=1e-6):
        shape = x.shape
        B, C = shape[0], shape[1]
