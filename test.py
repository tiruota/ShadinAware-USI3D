...
parser.add_argument('-e', '--dir_edge', type=str, default='dataset/edge_iiw_dexi/',
                    help="input image path")
...
#                                     transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))    # Make sure the vaule range of input tensor be consistent to the training time
    ...
    for image_name in t_bar:
        ...
        image_edge = os.path.join(opts.dir_edge, os.path.splitext(os.path.basename(image_name))[0]+'.png')
        ...
        edge = Variable(transform(Image.open(image_edge)).unsqueeze(0).cuda())

        # Start testing
        im_reflect, im_shading = intrinsic_image_decompose(image_cuda, edge, wo_fea)
