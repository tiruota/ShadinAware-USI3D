...
class CGIFolder(data.Dataset):
    ...
    def __init__(self, params, loader=default_loader, dict_image=None, is_train=True):
        if dict_image is None:
            dict_image = {'input': None, "edge-input": None, 'albedo': None, 'edge-albedo': None ,'shading': None, 'mask': None}
    ...
    def load_images(self, index, use_da=True):
        ...
        len_in_edge = len(self.img_dict['edge-input'])
        len_ab = len(self.img_dict['albedo'])
        len_ab_edge = len(self.img_dict['edge-albedo'])
        ...
        img_in_edge = np.float32(self.loader(self.img_dict['edge-input'][index % len_in_edge])) / 255.
        img_in_edge = img_in_edge[:, :, np.newaxis]
        img_ab = np.float32(self.loader(self.img_dict['albedo'][index % len_ab])) / 255.
        img_ab_edge = np.float32(self.loader(self.img_dict['edge-albedo'][index % len_ab_edge])) / 255.
        img_ab_edge = img_ab_edge[:, :, np.newaxis]
        ...
        fname_e_i = os.path.basename(self.img_dict['edge-input'][index % len_in_edge])
        fname_r = os.path.basename(self.img_dict['albedo'][index % len_ab])
        fname_e_r = os.path.basename(self.img_dict['edge-albedo'][index % len_ab_edge])
        # fname_s = os.path.basename(self.img_dict['shading'][index % len_in])
        ...
        if use_da:
            ...
            img_in_edge = self.data_argument(img_in_edge, 1, random_pos, random_flip)
            img_ab = self.data_argument(img_ab, 1, random_pos, random_flip)
            img_ab_edge = self.data_argument(img_ab_edge, 1, random_pos, random_flip)
            img_sd = self.data_argument(img_sd, 1, random_pos, random_flip)
            img_mk = self.data_argument(img_mk, 1, random_pos, random_flip)

        return img_in, img_in_edge, img_ab, img_ab_edge, img_sd, img_mk, file_name, fname_e_i, fname_r, fname_e_r
    ...
    def __getitem__(self, index):
        ...
        img_in, img_in_edge, img_ab, img_ab_edge, img_sd, img_mk, file_name, fname_e_i, fname_r, fname_e_r = self.load_images(index, self.is_train)
        ...
        img_edge = torch.from_numpy(np.transpose(img_in_edge, (2, 0, 1))).contiguous().float()
        ...
        targets['edge-albedo'] = torch.from_numpy(np.transpose(img_ab_edge, (2, 0, 1))).contiguous().float()
        targets['shading'] = torch.from_numpy(np.transpose(img_sd, (2, 0, 1))).contiguous().float()
        targets['name_i'] = file_name
        targets['name_e_i'] = fname_e_i
        targets['name_r'] = fname_r
        targets['name_e_r'] = fname_e_r
        ...
        return img_in, img_edge, targets
