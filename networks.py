...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, params, type=None):
        ...
        if type == 'reflectance':
            self.dec = ReflectanceDecoder(n_res, 1, self.enc_content.output_dim, output_dim,res_norm='adain', activ=activ,
                            pad_type=pad_type)
        else:
    ...
    def encode(self, images):
        ...
        # print(style_fake.shape, content.shape)
        return content, style_fake

    def decode(self, content, style, edge=None):
        # decode content and style codes to an image
        adain_params = self.mlp(style)
        self.assign_adain_params(adain_params, self.dec)
        if self.type == 'reflectance':
            images = self.dec(content, edge)
        else:
...
class ReflectanceDecoder(nn.Module):
    def __init__(self, n_res, edge_dim, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):# dim = 256
        super(ReflectanceDecoder, self).__init__()

        self.model_adain = []
        self.model_upsample1 = []
        self.model_upsample2 = []
        self.model_pad = []
        self.model_edge = []
        

        # AdaIN residual blocks
        self.model_adain = nn.Sequential(
                                        ResBlocks(n_res, dim, res_norm, activ, pad_type=pad_type))
        self.model_upsample1 = nn.Sequential(
                                nn.Upsample(scale_factor=2),
                                Conv2dBlock(512*2, 512, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type),
                                Conv2dBlock(512, 256, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type))
        self.model_upsample2 = nn.Sequential(
                                nn.Upsample(scale_factor=2),
                                Conv2dBlock(256*2, 256, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type),
                                Conv2dBlock(256, 128, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type))
        self.model_upsample3 = nn.Sequential(
                                nn.Upsample(scale_factor=2),
                                Conv2dBlock(128*2, 128, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type),
                                Conv2dBlock(128, 64, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type))
        self.model_pad = nn.Sequential(
                                Conv2dBlock(64*2, 64, 5, 1, 2, norm='ln', activation=activ, pad_type=pad_type),
                                Conv2dBlock(64, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type=pad_type))

        self.enc_edge1 = nn.Sequential(
            Conv2dBlock(edge_dim, 64, 7, 1, 3, norm='in', activation='lrelu', pad_type='reflect'),
            ResBlock(64, norm='in', activation='lrelu', pad_type='reflect')
        )
        self.enc_edge2 = nn.Sequential(
            Conv2dBlock(64, 128, 4, 2, 1, norm='in', activation='lrelu', pad_type='reflect'),
            ResBlock(128, norm='in', activation='lrelu', pad_type='reflect')
        )
        self.enc_edge3 = nn.Sequential(
            Conv2dBlock(128, 256, 4, 2, 1, norm='in', activation='lrelu', pad_type='reflect'),
            ResBlock(256, norm='in', activation='lrelu', pad_type='reflect')
        )
        self.enc_edge4 = nn.Sequential(
            Conv2dBlock(256, 512, 4, 2, 1, norm='in', activation='lrelu', pad_type='reflect'),
            ResBlock(512, norm='in', activation='lrelu', pad_type='reflect')
        )
        

    def forward(self, x, e):
        e1 = self.enc_edge1(e)
        e2 = self.enc_edge2(e1)
        e3 = self.enc_edge3(e2)
        e4 = self.enc_edge4(e3)

        c0 = self.model_adain(x)
        c1 = self.model_upsample1(torch.cat((c0, e4), 1))
        c2 = self.model_upsample2(torch.cat((c1, 0.7*e3), 1))
        c3 = self.model_upsample3(torch.cat((c2, 0.5*e2), 1))
        out = self.model_pad(torch.cat((c3, 0.2*e1), 1))
        return out

class Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'): # dim = 256
...
class IntrinsicMerger(nn.Module):
    def __init__(self, feature_len, hidden_dim, n_layers, activ):
        ...
        # self.mlp = MLPMixer(image_size=feature_len * 2, channels=1, patch_size=16, dim=512, depth=12, num_classes=feature_len)
...
class IntrinsicSplitor(nn.Module):
    def __init__(self, feature_len, hidden_dim, n_layers, activ):
        ...
        # self.mlp = MLPMixer(image_size=8, channels=1, patch_size=1, dim=512, depth=12, num_classes=self.feature_dim * 2)
    def forward(self, fea):
        # fea_shape = fea.shape
        # fea = fea.view(1, 1, 8, 8)
