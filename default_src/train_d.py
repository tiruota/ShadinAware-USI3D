...
from utils_d import get_local_time
from trainer_d import UnsupIntrinsicTrainer
from utils_d import prepare_sub_folder, write_html, write_loss, get_config, write_2images, Timer
from utils_d import get_intrinsic_data_loader
import numpy as np
...
for epoch in range(config['n_epoch']):
    for it, (image_in, targets) in enumerate(train_loader):
        ...
        if (iterations + 1) % config['image_save_iter'] == 0:
            with torch.no_grad():
                if 'MIX' not in opts.config:
                    test_image_outputs = trainer.sample(train_display_images_i, train_display_images_r, train_display_images_s)
